...
class QuestionsMcScreen extends Screen
{
    ...
    {
        return [
            'items' => Questions::filters()->defaultSort('updated_at', 'desc')->where('entity', 'mc')->paginate(10),
        ];
    }
    ...
    public function commandBar(): iterable
    {
        return [
            Link::make('Создать')
                ->icon('pencil')
                ->route('platform.questions_mc.create')
        ];
    }
            ...
            Layout::table('items', [
                TD::make('id')->sort()->render(function(Questions $item) {
                    return Link::make($item->id)
                        ->route('platform.questions_mc.edit', $item);
                }),
