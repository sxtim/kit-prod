...
class QuestionsMainScreen extends Screen
{
    /**
    ...
    {
        return [
            'items' => Questions::filters()->defaultSort('updated_at', 'desc')->where('entity', 'main')->paginate(10),
        ];
    }
    ...
    public function commandBar(): iterable
    {
        return [
            Link::make('Создать')
                ->icon('pencil')
                ->route('platform.questions_main.create')
        ];
    }
            ...
            Layout::table('items', [
                TD::make('id')->sort()->render(function(Questions $item) {
                    return Link::make($item->id)
                        ->route('platform.questions_main.edit', $item);
                }),
